...
def ACVAE_Interim_Phonemes(num_onset, num_nucleus, latent_dim, lr) -> tf.keras.Model:
    """
    This method builds and returns a Model
    :return:
    """
    ...
    x = tf.keras.layers.MaxPool2D(pool_size=(2,2), strides=None, padding='valid', data_format=None)(x)
    ...
    x = tf.keras.layers.MaxPool2D(pool_size=(2,2), strides=None, padding='valid', data_format=None)(x)
    x = tf.keras.layers.Conv2D(filters=128, kernel_size=(3,3), strides=(1,1), activation='relu', padding='same')(x)
    x = tf.keras.layers.MaxPool2D(pool_size=(2,2), strides=None, padding='valid', data_format=None)(x)
    x = tf.keras.layers.Flatten()(x)
    x = tf.keras.layers.Dense(latent_dim)(x)
    #x = tf.keras.layers.BatchNormalization()(x)
...
def CNN_Interim_Phonemes(num_onset, num_nucleus, latent_dim, lr) -> tf.keras.Model:
    ...
    x_onset = tf.keras.layers.Dense(num_onset, activation='softmax', name="onset")(z)
    x_nucleus = tf.keras.layers.Dense(num_nucleus, activation='softmax', name="nucleus")(z)
    
    optimizer = tf.keras.optimizers.Adam(lr)

    model = tf.keras.Model(inputs=[x_input], outputs=[reconstruction, x_onset, x_nucleus])
    model.compile(optimizer=optimizer, metrics=['accuracy'],
                  loss={"rec": tf.keras.losses.MeanSquaredError(),
                        "onset": tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
                        "nucleus": tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True)})
